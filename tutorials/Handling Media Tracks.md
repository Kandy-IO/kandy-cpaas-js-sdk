...
client.on('call:newTrack', function (params) {
  ...
  if (localTracks.length > 0 && call.localOp.operation !== 'ADD_BASIC_MEDIA') {
    localTracks.forEach(trackId => {
      // Gets the trackId's for each local tracks
      const track = client.media.getTrackById(trackId)
      let id = track.trackId

      // To avoid adding the same trackId more than once
      if (!allTracks.includes(id)) {
        allTracks.push(id)
        ...
        // Setting the attributes for the html elements created
        listAllTracks.innerHTML = track.kind + ':' + ' ' + id
        listAllTracks.id = id
        input.type = 'checkbox'
        input.id = id
        input.value = id
