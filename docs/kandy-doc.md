...
*   `call` **[Object][7]** The call configuration object.
    ...
    *   `call.sdpHandlers` **[Array][16]<[call.SdpHandlerFunction][17]>?** List of SDP handler functions to modify SDP. Advanced usage.
...
*   `mediaConnectionState` **[string][8]** The current status of the call's media connection. See [call.mediaConnectionStates][25] for possible states.
...
*   `localTracks` **[Array][16]<[string][8]>** A list of Track IDs that the call is sending to the remote participant.
*   `remoteTracks` **[Array][16]<[string][8]>** A list of Track IDs that the call is receiving from the remote participant.
*   `mediaOffered` **[call.MediaOffered][26]?** Information about what media was offered by the person who made the call.
...
*   `iceServers` **[Array][16]\<RTCIceServer>?** The list of ICE servers to be used for calls.
...
*   `urls` **([Array][16]<[string][8]> | [string][8])** Either an array of URLs for reaching out several ICE servers or a single URL for reaching one ICE server. See [RTCIceServers.urls documentation][30] to learn more about the actual url format.
...
*   `tracks` **[Array][16]<[call.TrackObject][32]>** A list of Track objects that are contained in this Media object.
...
*   `containers` **[Array][16]<[string][8]>** The list of CSS selectors that were used to render this Track.
...
*   `camera` **[Array][16]<[call.DeviceInfo][33]>** A list of camera device information.
*   `microphone` **[Array][16]<[call.DeviceInfo][33]>** A list of microphone device information.
*   `speaker` **[Array][16]<[call.DeviceInfo][33]>** A list of speaker device information.
...
Starts an outgoing call to a [UserID][34] or a
[PhoneNumber][35].
...
The SDK will emit a [call:start][37] event
locally when the operation completes. When the remote participant
receives the call, a [call:receive][38]
...
*   `destination` **([call.UserID][39] | [call.PhoneNumber][40])** The desired destination.
*   `media` **[Object][7]** The media options the call should be initialized with.
    ...
    *   `media.audioOptions` **[Object][7]?** Options for configuring the call's audio.
        ...
        *   `media.audioOptions.deviceId` **[call.MediaConstraint][41]?** ID of the microphone to receive audio from.
    ...
    *   `media.videoOptions` **[Object][7]?** Options for configuring the call's video.
        ...
        *   `media.videoOptions.deviceId` **[call.MediaConstraint][41]?** ID of the camera to receive video from.
        *   `media.videoOptions.height` **[call.MediaConstraint][41]?** The height of the video.
        *   `media.videoOptions.width` **[call.MediaConstraint][41]?** The width of the video.
        *   `media.videoOptions.frameRate` **[call.MediaConstraint][41]?** The frame rate of the video.
    ...
    *   `media.screenOptions` **[Object][7]?** Options for configuring the call's screenShare.
        ...
        *   `media.screenOptions.height` **[call.MediaConstraint][41]?** The height of the screenShare.
...
[call:newTrack][44] event for more
...
[call:stateChange][43] event as well.
...
call can be taken off hold with the [call.unhold][45] API.
...
Returns **[Array][16]<[call.CallObject][46]>** Call objects.
...
Returns **[call.CallObject][46]** A call object.
...
[call:trackEnded][47] event for more
information.
...
The SDK will emit a [call:newTrack][44] event
...
The SDK will emit a [call:trackEnded][47]
...
*   `tracks` **[Array][16]** A list of track IDs to remove.
*   `options` **[Object][7]?**  (optional, default `{}`)
...
[call.addMedia][48] API can be used.
...
*   `videoOptions` **[Object][7]?** Options for configuring the call's video.
    ...
    *   `videoOptions.deviceId` **[call.MediaConstraint][41]?** ID of the camera to receive video from.
    *   `videoOptions.height` **[call.MediaConstraint][41]?** The height of the video.
    *   `videoOptions.width` **[call.MediaConstraint][41]?** The width of the video.
    *   `videoOptions.frameRate` **[call.MediaConstraint][41]?** The frame rate of the video.
*   `options` **[Object][7]?** 
...
[call.removeMedia][49] API can be used.
...
The [call.addMedia][48] API can be used to perform the same behaviour
...
[call.addMedia][48] will perform the same behaviour as using
`startScreenshare`.
...
The [call.removeMedia][49] API can be used to perform the same
...
track when using [call.removeMedia][49] will perform the same
...
The data retrieved is a [RTCStatsReport][50]
...
The SDK will emit a
[call:statsReceived][52] event, after
...
In addition, the SDK emits a [call:availableCodecs][54] event
upon retrieving that list of codecs.

This API is a wrapper for the static method [RTCRtpSender.getCapabilities()][55].
...
Set [SDP Handler Functions][56] that will be run as part of a pipeline for all future calls.
...
*   `sdpHandlers` **[Array][16]<[call.SdpHandlerFunction][17]>** The list of SDP handler functions to modify SDP.
...
The Call's state is a property of the [CallObject][57],
which can be retrieved using the [call.getById][22] or
[call.getAll][21] APIs.

The SDK emits a [call:stateChange][43]
...
state can be used alongside the [Call state][24] to determine if media issues
...
problem. Using the [call.restartMedia][58] API will attempt to reconnect the media. See
the [call.restartMedia][58] API description for more information.

These states are direct reflections of the possible
[RTCPeerConnection.iceConnectionState][59] values.

The Call's media connection state is a property of the [CallObject][57],
which can be retrieved using the [call.getById][22] or
[call.getAll][21] APIs.

The SDK emits a [call:mediaConnectionChange][60]
...
*   `FAILED` **[string][8]** The connection has failed and cannot be recovered automatically. A full media connection refresh is required to reestablish a connection. See the [call.restartMedia][58] API.
...
necessary after the Call's [mediaConnectionState][25]
...
[mediaConnectionState][25] should have
...
and will remain in the `failed` [mediaConnectionState][25].
...
[mediaConnectionState][25] no matter what
...
[call:mediaConnectionChange][60]
event emitted. For this reason, Chrome-based applications may need a
short delay after receiving the [call:mediaRestart][61]
event before checking the Call's updated
[mediaConnectionState][25] to ensure the
application is acting on the "latest" state.

The SDK will emit a [call:mediaRestart][61]
...
[call.make][62] and [call.answer][63] APIs, and speaker can be
chosen when the audio track is rendered with the
[media.renderTracks][64] API.
...
using the general-purpose [call.replaceTrack][65] API. This API can
...
by unrendering the track with [media.removeTracks][66], then
re-rendering it with a new speaker with [media.renderTracks][64].
...
information (see the [call.getById][22] API). This event indicates that
...
*   `params` **[Object][7]** 
    ...
    *   `params.error` **[api.BasicError][20]?** An error object, if the operation was not successful.

### call:start

An outgoing call has been started.

Information about the Call can be retrieved using the [call.getById][22]
API.
...
See [call.states][24] for information about call states.
...
*   `params` **[Object][7]** 
    ...
    *   `params.tracks` **[Array][16]** The list of new Tracks.
...
*   `params` **[Object][7]** 
    ...
    *   `params.tracks` **[Array][16]** The list of removed Tracks.
...
the [call.removeMedia][49] API. Tracks are also removed from Calls
...
its media removed, see [call:removedMedia][68]
...
See the [call.getStats][69] API for more information.
...
*   `params` **[Object][7]** 
    ...
    *   `params.result` **[Map][70]** The RTCStatsReport.
    *   `params.error` **[api.BasicError][20]?** An error object, if the operation was not successful.
...
A Track is replaced by the local user using the [call.replaceTrack][65]
API.

This event is similar to the [call:newTrack][44]
...
*   `params` **[Object][7]** 
    ...
    *   `params.oldTrack` **[call.TrackObject][32]?** State of the replaced track.
    *   `params.error` **[api.BasicError][20]?** An error object, if the operation was not successful.
...
This event is emitted as a result of the [call.getAvailableCodecs][71] API. Please refer to the API for more
information.
...
*   `params` **[Object][7]** 
    ...
    *   `params.codecs` **[Array][16]<[Object][7]>** The list of codecs.
...
To check the media connection state of a call, retrieve the call's information using the  [call.getById][22] API,
...
This event is emitted as a result of the [call.restartMedia][58] API being called.
See the description for [call.restartMedia][58] for information about its
usage.

The [call:mediaConnectionChange][60] event
...
Can be retrieved using the [connection.getSocketState][72] API.
...
Returns **[connection.WSConnectionObject][73]** Details about the current websocket connection, including state and configuration.
...
Returns **[Array][16]** List of contact information.
...
Available conversations can be retrieved using the [conversation.get][74]
or [conversation.getAll][75] APIs.

Messaging functions are all part of the 'conversation' namespace. Ex: [client.conversation.getAll][76]
...
If successful, the event [conversations:change][77] will be emitted.

Available conversation information can be retrieved using the
[conversation.get][74] or [conversation.getAll][75] Messaging
APIs.
...
*   `options` **[Object][7]?** A configuration object to query for more specific results.
    ...
    *   `options.type` **[string][8]** The type of conversation to fetch. See [conversation.chatTypes][78] for valid types. (optional, default `'chat-oneToOne'`)

### get

Retrieves a conversation object matching the User ID and Type provided if available.

Conversations are made available using the [conversation.fetch][79] or
[conversation.create][80] Messaging APIs.
...
*   `options` **[Object][7]?** Options used to query for more specific results.
    ...
    *   `options.type` **[string][8]** The type of conversation to get. See [conversation.chatTypes][78] for valid types. (optional, default `'chat-oneToOne'`)

Returns **[conversation.Conversation][81]** A Conversation object.
...
If successful, the event [conversations:new][82] will be emitted.
...
*   `options` **[Object][7]?** Options to use when creating a new conversation object.
    ...
    *   `options.type` **[string][8]** The type of conversation to create. See [conversation.chatTypes][78] for valid types. (optional, default `'chat-oneToOne'`)
...
Returns **[Array][16]<[conversation.Conversation][81]>** An array of Conversation objects.
...
user and a group. A user can create a Conversation using [conversation.create][80] Messaging API.

A Conversation can be used to create messages to send using the Conversation and Messaging APIs
[Conversation.createMessage][83] and [Message.send][84] functions.
...
*   `type` **[string][8]** The type of conversation. See [conversation.chatTypes][78] for valid types.
*   `lastMessage` **[string][8]** The last message received.
*   `messages` **[Array][16]<[conversation.Message][85]>** The array of message objects.
*   `isTypingList` **[Array][16]<[string][8]>** The array identifying the User IDs of the users who are currently typing.
...
If successful, the event [messages:change][86] will be emitted.

##### Parameters

*   `part` **[conversation.Part][87]** The Part to add to the message.
...
Returns **[conversation.Message][85]** The newly created Message object.
...
Returns **[Array][16]<[conversation.Message][85]>** An array of messages.
...
Returns **[conversation.Message][85]** A message object.
...
*   `messageIds` **[Array][16]<[string][8]>** An array of message IDs for the messages to be deleted (optional, default `[]`)
...
Messages can then be retrieved using [Conversation.getMessages][88].
...
If successful, the event [isTypingList:change][89] will be emitted.
...
The message object can be obtained through the [Conversation.createMessage][83] API on an existing conversation.

Messages have Parts which represent pieces of a message, such as a text part, a json object part or a file part.
Once all the desired parts have been added to the message using the [Message.addPart][90] function,
the message can then be sent using the [Message.send][84] function.
...
obtained through the [Conversation.getMessages][88] or [Conversation.getMessage][91] API on an existing conversation.
...
*   `parts` **[Array][16]<[conversation.Part][87]>** An array of Part Objects.
*   `sender` **[string][8]** The primary contact address of the sender.
*   `destination` **[Array][16]<[string][8]>** An array of primary contact addresses associated with various destinations to which the message is meant to be delivered.
...
*   `type` **[string][8]** The type of message that was sent. See [conversation.chatTypes][78] for valid types.
...
*   `params` **[Array][16]<[Object][7]>** An array containing one object with information about the newly created conversation.

    *   `params.destination` **[Array][16]<[string][8]>** An array of destinations for messages created in this conversation.
    *   `params.type` **[string][8]** The type of conversation created. See [conversation.chatTypes][78] for valid types.
...
*   `params` **[Array][16]<[Object][7]>** An array of objects containing information about the conversations that have changed.

    *   `params.destination` **[Array][16]<[string][8]>** An array of destinations for messages in this conversation.
    *   `params.type` **[string][8]** The type of conversation changed. See [conversation.chatTypes][78] for valid types.
...
*   `params` **[Object][7]** 
    ...
    *   `params.destination` **[Array][16]<[string][8]>** An array of destinations for the message affected.
    *   `params.type` **[string][8]** The type of conversation. See [conversation.chatTypes][78] for valid types.
...
*   `params` **[Object][7]** 
    ...
    *   `params.destination` **[Array][16]<[string][8]>** An array of destinations in this conversation.
...
[Messaging][92] feature to allow for group conversations.

Groups are persisted by the server. When the SDK is initialized, there will
be no Group information available, but the [groups.fetch][93]
...
The SDK will emit a [group:new][94] event locally
...
using the [groups.addParticipant][95] and
[groups.removeParticipant][96] APIs.

Group information will become available after the operation completes
using the [groups.get][97] and [groups.getAll][98] APIs.

#### Parameters

*   `params` **[Object][7]** 

    *   `params.participants` **[Array][16]<[string][8]>?** List of participants to
...
The SDK will emit a [group:refresh][99] event
when the operation completes.

Information about an available Group can be retrieved using the
[groups.getAll][98] or [groups.get][97] APIs.

### getAll

Retrieves information about all available Groups.

Returns **[Array][16]\<Group>** The list of all available Groups.
...
Returns **[Array][16]<[string][8]>** The list of Group participants.

### getInvitations

Retrieves information about all Group invitations available.

The [group:invitation_received][100]
event indicates that a new Group invitation is available.

Returns **[Array][16]\<Invitations>** The list of Group invitations.

### leave

Leaves a Group.

The SDK will emit a [group:change][101] event
when the operation completes.
...
a [group:invitation_received][100]
event.
...
The Group will no longer be available using the [groups.get][97]
and [groups.getAll][98] APIs.

The SDK will emit a [group:delete][102] event
...
*   `params` **[Object][7]** 
    ...
    *   `params.group` **[Object][7]** The information about the Group that was deleted.
        ...
        *   `params.group.participants` **[Array][16]** the participants of the group of the deleted group
...
*   `params` **[Object][7]** 
    ...
    *   `params.invitation` **[Object][7]** contains information about the group invitation
        ...
        *   `params.invitation.participants` **[Array][16]** a list of participants in the group
...
[config.logs][104]) to customize how the logs are
...
[config.logs][104]) will need to handle LogEntry
objects.
...
*   `messages` **[Array][16]** The logged information, given to the Logger
...
[config.logs][104]). The SDK will then provide this
...
SDK has access to. Media is used in conjunction with the [Calls][105]
...
the [media.getDevices][106] API.

### getDevices

Retrieves the available media devices for use.

The [devices:change][107] event will be
...
Returns **[call.MediaObject][108]** A Media object.
...
For device information, the [media.getDevices][106] API will retrieve
...
recommend using the [media.initializeDevices][109] API in this
...
[devices:error][110] event if an error is
encountered.
...
*   `trackIds` **[Array][16]<[string][8]>** List of Track IDs to be rendered.
...
*   `trackIds` **[Array][16]<[string][8]>** List of Track IDs to stop being rendered.
*   `cssSelector` **[string][8]** A CSS selector string that uniquely
    identifies an element. Ensure that special characters are properly
    escaped.
...
The SDK will emit a [media:muted][111] event
when a Track has been muted.

#### Parameters

*   `trackIds` **[Array][16]<[string][8]>** List of Track IDs.
...
The SDK will emit a [media:unmuted][112] event
...
[MediaConstraint(s)][113] that was provided.

#### Parameters

*   `params` **[Object][7]** 

    *   `params.error` **[Error][19]** The Basic error object.

### media:muted

The specified Tracks have been muted.

A Track can be muted using the [media.muteTracks][114] API.

#### Parameters

*   `params` **[Object][7]** 

    *   `params.tracks` **[Array][16]<[string][8]>** The list of Tracks that were muted.

### media:unmuted

The specified Tracks have been unmuted.

A Track can be unmuted using the [media.unmuteTracks][115]
API.
...
*   `params` **[Object][7]** 
    ...
    *   `params.tracks` **[Array][16]<[string][8]>** The list of Tracks that were unmuted.
...
[media:sourceMuted][117] event), and will
...
*   `params` **[Object][7]** 
    ...
    *   `params.trackIds` **[Array][16]<[string][8]>** The list of track id's that were rendered.
...
available either by using [presence.fetch][118] or by subscribing for
updates about other Users, using [presence.subscribe][119].

Available presence information can be retrieved using [presence.get][120] or
[presence.getAll][121].
...
An instance of this type can be obtained by invoking the [presence.get][120] function.
...
The status is set to [open][122] as soon as a user subscribes for the presence service.
...
*   `status` **[string][8]** The current status the user has set for themselves. For supported values see [presence.statuses][122].
*   `activity` **[string][8]** The current activity of the user.
    For supported values see [presence.activities][123].
...
See [presence.statuses][122] and [presence.activities][123] for valid
values.

The SDK will emit a
[presence:selfChange][124] event
when the operation completes. The updated presence information is
available and can be retrieved with [presence.getSelf][125].

Other users subscribed for this user's presence will receive a
[presence:change][126] event.
...
*   `user` **([Array][16]<[string][8]> | [string][8])** A User ID or an array of User IDs.

Returns **([Array][16]<[Object][7]> | [Object][7])** List of user presence information.

### getAll

Retrieves the presence information for all available users.

Returns **[Array][16]<[Object][7]>** List of user presence information.

### getSelf

Retrieves the presence information for the current user.

This information is set using the [presence.update][127] API.
...
[presence.get][120] or [presence.getAll][121] APIs.
...
*   `users` **([Array][16]<[string][8]> | [string][8])** A User ID or an array of User IDs.
...
This event is generated as a result of [presence.fetch][118] or [presence.update][127] operations.
...
The changed information can be retrieved using the [presence.get][120]
API.
...
The changed information can be retrieved using the [presence.getSelf][125]
API.
...
*   `params` **[Object][7]** A subscription object containing data.
    ...
    *   `params.userIds` **[Array][16]<[string][8]>** The ID(s) of the user(s) whose presence needs to be watched.
...
*   `params` **[Object][7]** A subscription object containing data.
    ...
    *   `params.userIds` **[Array][16]<[string][8]>** The ID(s) of the user(s) whose presence no longer requires to be watched.
...
This is a wrapper on top of the browser's [fetch API][128]
...
*   `init` **RequestInit** An object containing any custom settings that you want to apply to the request. See [fetch API][128]
...
Returns **[Promise][129]<[Response][130]>** A promise for a [Response][131] object.

## sdpHandlers

A set of [SdpHandlerFunction][56]s for manipulating SDP information.
...
*   `fmtpParams` **[Array][16]<[string][8]>** An array of strings to match against the "a=fmtp" format parameters for the corresponding codec.
...
removal handler in combination with the [call.getAvailableCodecs][71] function in order to build logic to determine the best codecs to use
for their application.

#### Parameters

*   `codecs` **([Array][16]\<CodecSelector> | [Array][16]<[string][8]>)** A list of codec selectors to remove from the SDP. If passing a list of strings, they will be converted into
...
Returns **[call.SdpHandlerFunction][17]** The resulting SDP handler that will remove the codec.
...
with the [services.subscribe][135] API.
...
the [services.subscribe][135] function.
...
This is the configuration object that needs to be passed as the value for the [ServiceDescriptor.params][136] property.
...
[services.getSubscriptions][134] for information about services.
...
*   `services` **[Array][16]<([string][8] | [services.ServiceDescriptor][137])>** A list of service configurations.
...
*   `services` **[Array][16]<[string][8]>** A list of subscribed service names.
...
The SDK will emit a [users:change][138]
...
This API is simply a shortcut for the [user.fetch(getUserInfo().identity)][140] API.
...
See the [user.fetch][140] and [user.search][141] APIs for details about
...
Returns **[user.User][142]** The User object for the specified user.
...
Returns **[Array][16]<[user.User][142]>** An array of all the User objects.

### search

Searches the domain's directory for Users.

The SDK will emit a [directory:change][143]
...
[user.get][139] and [user.getAll][144] APIs.
...
*   `params` **[Object][7]** 
    ...
    *   `params.results` **[Array][16]<[user.User][142]>** The Users' information returned by the
        operation.
...
[25]: #callmediaconnectionstates

[26]: #callmediaoffered

[27]: #callbandwidthcontrols

[28]: https://www.w3.org/TR/webrtc-priority/#rtc-priority-type

[29]: https://developer.mozilla.org/en-US/docs/Web/API/RTCIceServer#properties

[30]: https://developer.mozilla.org/en-US/docs/Web/API/RTCIceServer/urls
...
[37]: #calleventcallstart

[38]: #calleventcallreceive

[39]: #calluserid

[40]: #callphonenumber

[41]: #callmediaconstraint

[42]: #calldscpcontrols

[43]: #calleventcallstatechange

[44]: #calleventcallnewtrack

[45]: #callunhold

[46]: #callcallobject

[47]: #calleventcalltrackended

[48]: #calladdmedia

[49]: #callremovemedia

[50]: https://developer.mozilla.org/en-US/docs/Web/API/RTCStatsReport

[51]: https://developer.mozilla.org/en-US/docs/Web/API/RTCStats

[52]: #calleventcallstatsreceived

[53]: #calleventcalltrackreplaced

[54]: #calleventcallavailablecodecs

[55]: https://w3c.github.io/webrtc-pc/#dom-rtcrtpsender-getcapabilities
...
[59]: https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/iceConnectionState

[60]: #calleventcallmediaconnectionchange

[61]: #calleventcallmediarestart

[62]: #callmake

[63]: #callanswer

[64]: #mediarendertracks

[65]: #callreplacetrack

[66]: #mediaremovetracks

[67]: #mediagettrackbyid

[68]: #calleventcallremovedmedia

[69]: #callgetstats

[70]: https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Map

[71]: #callgetavailablecodecs

[72]: #connectiongetsocketstate

[73]: #connectionwsconnectionobject

[74]: #conversationget

[75]: #conversationgetall

[76]: messaging.getAll

[77]: #conversationeventconversationschange

[78]: #conversationchattypes

[79]: #conversationfetch

[80]: #conversationcreate

[81]: #conversationconversation

[82]: #conversationeventconversationsnew

[83]: #conversationconversationcreatemessage

[84]: #conversationmessagesend

[85]: #conversationmessage

[86]: #conversationeventmessageschange

[87]: #conversationpart

[88]: #conversationconversationgetmessages

[89]: #conversationeventistypinglistchange

[90]: #conversationmessageaddpart

[91]: #conversationconversationgetmessage

[92]: #conversation

[93]: #groupsfetch

[94]: #groupseventgroupnew

[95]: #groupsaddparticipant

[96]: #groupsremoveparticipant

[97]: #groupsget

[98]: #groupsgetall

[99]: #groupseventgrouprefresh

[100]: #groupseventgroupinvitation_received

[101]: #groupseventgroupchange

[102]: #groupseventgroupdelete

[103]: #groupsgetinvitations

[104]: #configconfiglogs

[105]: #call

[106]: #mediagetdevices

[107]: #mediaeventdeviceschange

[108]: #callmediaobject

[109]: #mediainitializedevices

[110]: #mediaeventdeviceserror

[111]: #mediaeventmediamuted

[112]: #mediaeventmediaunmuted

[113]: #callmediaconstraint

[114]: #mediamutetracks

[115]: #mediaunmutetracks

[116]: #mediaeventmediasourceunmuted

[117]: #mediaeventmediasourcemuted

[118]: #presencefetch

[119]: #presencesubscribe

[120]: #presenceget

[121]: #presencegetall

[122]: #presencestatuses

[123]: #presenceactivities

[124]: #presenceeventpresenceselfchange

[125]: #presencegetself

[126]: #presenceeventpresencechange

[127]: #presenceupdate

[128]: https://developer.mozilla.org/en-US/docs/Web/API/fetch

[129]: https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise

[130]: https://developer.mozilla.org/docs/Web/Guide/HTML/HTML5
