...
*   `state` **[string][8]** The current state of the call. See [call.states][23] for possible states.
*   `mediaConnectionState` **[string][8]** The current status of the call's media connection. See [call.mediaConnectionStates][24] for possible states.
...
*   `mediaOffered` **[call.MediaOffered][25]?** Information about what media was offered by the person who made the call.
...
*   `urls` **([Array][13]<[string][8]> | [string][8])** Either an array of URLs for reaching out several ICE servers or a single URL for reaching one ICE server. See [RTCIceServers.urls documentation][28] to learn more about the actual url format.
...
*   `tracks` **[Array][13]<[call.TrackObject][30]>** A list of Track objects that are contained in this Media object.
...
*   `camera` **[Array][13]<[call.DeviceInfo][31]>** A list of camera device information.
*   `microphone` **[Array][13]<[call.DeviceInfo][31]>** A list of microphone device information.
*   `speaker` **[Array][13]<[call.DeviceInfo][31]>** A list of speaker device information.
...
Starts an outgoing call to a [UserID][32] or a
[PhoneNumber][33].
...
*   `destination` **([call.UserID][37] | [call.PhoneNumber][38])** The desired destination.
...
*   `options` **[Object][7]?** 
    ...
    *   `options.dscpControls` **[call.DSCPControls][40]?** Options for configuring DSCP markings on the media traffic
...
[call:newTrack][42] event for more
...
[call:stateChange][41] event as well.
...
call can be taken off hold with the [call.unhold][43] API.
...
Returns **[Array][13]<[call.CallObject][44]>** Call objects.
...
Returns **[call.CallObject][44]** A call object.
...
[call:trackEnded][45] event for more
information.
...
The SDK will emit a [call:trackEnded][45]
...
[call.addMedia][46] API can be used.
...
*   `videoOptions` **[Object][7]?** Options for configuring the call's video.
    ...
    *   `videoOptions.deviceId` **[call.MediaConstraint][39]?** ID of the camera to receive video from.
    *   `videoOptions.height` **[call.MediaConstraint][39]?** The height of the video.
    *   `videoOptions.width` **[call.MediaConstraint][39]?** The width of the video.
    *   `videoOptions.frameRate` **[call.MediaConstraint][39]?** The frame rate of the video.
*   `options` **[Object][7]?** 
...
[call.removeMedia][47] API can be used.
...
The [call.addMedia][46] API can be used to perform the same behaviour
...
[call.addMedia][46] will perform the same behaviour as using
`startScreenshare`.
...
The [call.removeMedia][47] API can be used to perform the same
...
track when using [call.removeMedia][47] will perform the same
...
The data retrieved is a [RTCStatsReport][48]
...
The SDK will emit a
[call:statsReceived][50] event, after
...
In addition, the SDK emits a [call:availableCodecs][52] event
upon retrieving that list of codecs.

This API is a wrapper for the static method [RTCRtpSender.getCapabilities()][53].
...
Set [SDP Handler Functions][54] that will be run as part of a pipeline for all future calls.
...
The SDK emits a [call:stateChange][41]
...
state can be used alongside the [Call state][23] to determine if media issues
...
problem. Using the [call.restartMedia][56] API will attempt to reconnect the media. See
the [call.restartMedia][56] API description for more information.

These states are direct reflections of the possible
[RTCPeerConnection.iceConnectionState][57] values.

The Call's media connection state is a property of the [CallObject][55],
which can be retrieved using the [call.getById][21] or
[call.getAll][20] APIs.

The SDK emits a [call:mediaConnectionChange][58]
...
*   `FAILED` **[string][8]** The connection has failed and cannot be recovered automatically. A full media connection refresh is required to reestablish a connection. See the [call.restartMedia][56] API.
...
necessary after the Call's [mediaConnectionState][24]
...
[mediaConnectionState][24] should have
...
and will remain in the `failed` [mediaConnectionState][24].
...
[mediaConnectionState][24] no matter what
...
[call:mediaConnectionChange][58]
event emitted. For this reason, Chrome-based applications may need a
short delay after receiving the [call:mediaRestart][59]
event before checking the Call's updated
[mediaConnectionState][24] to ensure the
application is acting on the "latest" state.

The SDK will emit a [call:mediaRestart][59]
...
[call.make][60] and [call.answer][61] APIs, and speaker can be
chosen when the audio track is rendered with the
[media.renderTracks][62] API.
...
using the general-purpose [call.replaceTrack][63] API. This API can
...
re-rendering it with a new speaker with [media.renderTracks][62].
...
information (see the [call.getById][21] API). This event indicates that
...
Information about the Call can be retrieved using the [call.getById][21]
API.
...
See [call.states][23] for information about call states.
...
the [call.removeMedia][47] API. Tracks are also removed from Calls
...
its media removed, see [call:removedMedia][66]
...
See the [call.getStats][67] API for more information.
...
*   `params` **[Object][7]** 
    ...
    *   `params.result` **[Map][68]** The RTCStatsReport.
    *   `params.error` **[api.BasicError][19]?** An error object, if the operation was not successful.
...
A Track is replaced by the local user using the [call.replaceTrack][63]
API.

This event is similar to the [call:newTrack][42]
...
*   `params` **[Object][7]** 
    ...
    *   `params.oldTrack` **[call.TrackObject][30]?** State of the replaced track.
    *   `params.error` **[api.BasicError][19]?** An error object, if the operation was not successful.
...
This event is emitted as a result of the [call.getAvailableCodecs][69] API. Please refer to the API for more
information.
...
To check the media connection state of a call, retrieve the call's information using the  [call.getById][21] API,
...
This event is emitted as a result of the [call.restartMedia][56] API being called.
See the description for [call.restartMedia][56] for information about its
usage.

The [call:mediaConnectionChange][58] event
...
Returns **[connection.WSConnectionObject][71]** Details about the current websocket connection, including state and configuration.
...
Available conversations can be retrieved using the [conversation.get][72]
or [conversation.getAll][73] APIs.

Messaging functions are all part of the 'conversation' namespace. Ex: [client.conversation.getAll][74]
...
If successful, the event [conversations:change][75] will be emitted.

Available conversation information can be retrieved using the
[conversation.get][72] or [conversation.getAll][73] Messaging
APIs.
...
*   `options` **[Object][7]?** A configuration object to query for more specific results.
    ...
    *   `options.type` **[string][8]** The type of conversation to fetch. See [conversation.chatTypes][76] for valid types. (optional, default `'chat-oneToOne'`)

### get

Retrieves a conversation object matching the User ID and Type provided if available.

Conversations are made available using the [conversation.fetch][77] or
[conversation.create][78] Messaging APIs.
...
*   `options` **[Object][7]?** Options used to query for more specific results.
    ...
    *   `options.type` **[string][8]** The type of conversation to get. See [conversation.chatTypes][76] for valid types. (optional, default `'chat-oneToOne'`)

Returns **[conversation.Conversation][79]** A Conversation object.
...
If successful, the event [conversations:new][80] will be emitted.
...
*   `options` **[Object][7]?** Options to use when creating a new conversation object.
    ...
    *   `options.type` **[string][8]** The type of conversation to create. See [conversation.chatTypes][76] for valid types. (optional, default `'chat-oneToOne'`)
...
Returns **[Array][13]<[conversation.Conversation][79]>** An array of Conversation objects.
...
user and a group. A user can create a Conversation using [conversation.create][78] Messaging API.

A Conversation can be used to create messages to send using the Conversation and Messaging APIs
[Conversation.createMessage][81] and [Message.send][82] functions.
...
*   `type` **[string][8]** The type of conversation. See [conversation.chatTypes][76] for valid types.
*   `lastMessage` **[string][8]** The last message received.
*   `messages` **[Array][13]<[conversation.Message][83]>** The array of message objects.
...
If successful, the event [messages:change][84] will be emitted.

##### Parameters

*   `part` **[conversation.Part][85]** The Part to add to the message.
...
Returns **[conversation.Message][83]** The newly created Message object.
...
Returns **[Array][13]<[conversation.Message][83]>** An array of messages.
...
Returns **[conversation.Message][83]** A message object.
...
Messages can then be retrieved using [Conversation.getMessages][86].
...
If successful, the event [isTypingList:change][87] will be emitted.
...
The message object can be obtained through the [Conversation.createMessage][81] API on an existing conversation.

Messages have Parts which represent pieces of a message, such as a text part, a json object part or a file part.
Once all the desired parts have been added to the message using the [Message.addPart][88] function,
...
obtained through the [Conversation.getMessages][86] or [Conversation.getMessage][89] API on an existing conversation.
...
*   `parts` **[Array][13]<[conversation.Part][85]>** An array of Part Objects.
...
*   `type` **[string][8]** The type of message that was sent. See [conversation.chatTypes][76] for valid types.
...
*   `params` **[Array][13]<[Object][7]>** An array containing one object with information about the newly created conversation.
    ...
    *   `params.type` **[string][8]** The type of conversation created. See [conversation.chatTypes][76] for valid types.
...
*   `params` **[Array][13]<[Object][7]>** An array of objects containing information about the conversations that have changed.
    ...
    *   `params.type` **[string][8]** The type of conversation changed. See [conversation.chatTypes][76] for valid types.
...
*   `params` **[Object][7]** 
    ...
    *   `params.type` **[string][8]** The type of conversation. See [conversation.chatTypes][76] for valid types.
...
[Messaging][90] feature to allow for group conversations.

Groups are persisted by the server. When the SDK is initialized, there will
be no Group information available, but the [groups.fetch][91]
...
The SDK will emit a [group:new][92] event locally
...
using the [groups.addParticipant][93] and
[groups.removeParticipant][94] APIs.

Group information will become available after the operation completes
using the [groups.get][95] and [groups.getAll][96] APIs.
...
The SDK will emit a [group:refresh][97] event
when the operation completes.

Information about an available Group can be retrieved using the
[groups.getAll][96] or [groups.get][95] APIs.
...
The [group:invitation_received][98]
...
The SDK will emit a [group:change][99] event
when the operation completes.
...
a [group:invitation_received][98]
event.
...
The Group will no longer be available using the [groups.get][95]
and [groups.getAll][96] APIs.

The SDK will emit a [group:delete][100] event
...
[config.logs][102]) will need to handle LogEntry
objects.
...
[config.logs][102]). The SDK will then provide this
...
SDK has access to. Media is used in conjunction with the [Calls][103]
...
the [media.getDevices][104] API.

### getDevices

Retrieves the available media devices for use.

The [devices:change][105] event will be
...
Returns **[call.MediaObject][106]** A Media object.
...
For device information, the [media.getDevices][104] API will retrieve
...
recommend using the [media.initializeDevices][107] API in this
...
The SDK will emit a [devices:change][105]
event when the operation is successful or a
[devices:error][108] event if an error is
encountered.
...
The SDK will emit a [media:muted][109] event
...
The SDK will emit a [media:unmuted][110] event
...
[MediaConstraint(s)][111] that was provided.
...
A Track can be muted using the [media.muteTracks][112] API.
...
A Track can be unmuted using the [media.unmuteTracks][113]
API.
...
[media:sourceMuted][115] event), and will
...
available either by using [presence.fetch][116] or by subscribing for
updates about other Users, using [presence.subscribe][117].

Available presence information can be retrieved using [presence.get][118] or
[presence.getAll][119].
...
An instance of this type can be obtained by invoking the [presence.get][118] function.
...
The status is set to [open][120] as soon as a user subscribes for the presence service.
...
*   `status` **[string][8]** The current status the user has set for themselves. For supported values see [presence.statuses][120].
*   `activity` **[string][8]** The current activity of the user.
    For supported values see [presence.activities][121].
...
See [presence.statuses][120] and [presence.activities][121] for valid
values.

The SDK will emit a
[presence:selfChange][122] event
when the operation completes. The updated presence information is
available and can be retrieved with [presence.getSelf][123].

Other users subscribed for this user's presence will receive a
[presence:change][124] event.
...
This information is set using the [presence.update][125] API.
...
[presence.get][118] or [presence.getAll][119] APIs.
...
This event is generated as a result of [presence.fetch][116] or [presence.update][125] operations.
...
The changed information can be retrieved using the [presence.get][118]
API.
...
The changed information can be retrieved using the [presence.getSelf][123]
API.
...
This is a wrapper on top of the browser's [fetch API][126]
...
*   `init` **RequestInit** An object containing any custom settings that you want to apply to the request. See [fetch API][126]
...
Returns **[Promise][127]<[Response][128]>** A promise for a [Response][129] object.

## sdpHandlers

A set of [SdpHandlerFunction][54]s for manipulating SDP information.
...
removal handler in combination with the [call.getAvailableCodecs][69] function in order to build logic to determine the best codecs to use
...
[45]: #calleventcalltrackended

[46]: #calladdmedia

[47]: #callremovemedia

[48]: https://developer.mozilla.org/en-US/docs/Web/API/RTCStatsReport

[49]: https://developer.mozilla.org/en-US/docs/Web/API/RTCStats

[50]: #calleventcallstatsreceived

[51]: #calleventcalltrackreplaced

[52]: #calleventcallavailablecodecs

[53]: https://w3c.github.io/webrtc-pc/#dom-rtcrtpsender-getcapabilities
...
[57]: https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/iceConnectionState

[58]: #calleventcallmediaconnectionchange

[59]: #calleventcallmediarestart

[60]: #callmake

[61]: #callanswer

[62]: #mediarendertracks

[63]: #callreplacetrack

[64]: #mediaremovetracks

[65]: #mediagettrackbyid

[66]: #calleventcallremovedmedia

[67]: #callgetstats

[68]: https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Map

[69]: #callgetavailablecodecs

[70]: #connectiongetsocketstate

[71]: #connectionwsconnectionobject

[72]: #conversationget

[73]: #conversationgetall

[74]: messaging.getAll

[75]: #conversationeventconversationschange

[76]: #conversationchattypes

[77]: #conversationfetch

[78]: #conversationcreate

[79]: #conversationconversation

[80]: #conversationeventconversationsnew

[81]: #conversationconversationcreatemessage

[82]: #conversationmessagesend

[83]: #conversationmessage

[84]: #conversationeventmessageschange

[85]: #conversationpart

[86]: #conversationconversationgetmessages

[87]: #conversationeventistypinglistchange

[88]: #conversationmessageaddpart

[89]: #conversationconversationgetmessage

[90]: #conversation

[91]: #groupsfetch

[92]: #groupseventgroupnew

[93]: #groupsaddparticipant

[94]: #groupsremoveparticipant

[95]: #groupsget

[96]: #groupsgetall

[97]: #groupseventgrouprefresh

[98]: #groupseventgroupinvitation_received

[99]: #groupseventgroupchange

[100]: #groupseventgroupdelete

[101]: #groupsgetinvitations

[102]: #configconfiglogs

[103]: #call

[104]: #mediagetdevices

[105]: #mediaeventdeviceschange

[106]: #callmediaobject

[107]: #mediainitializedevices

[108]: #mediaeventdeviceserror

[109]: #mediaeventmediamuted

[110]: #mediaeventmediaunmuted

[111]: #callmediaconstraint

[112]: #mediamutetracks

[113]: #mediaunmutetracks

[114]: #mediaeventmediasourceunmuted

[115]: #mediaeventmediasourcemuted

[116]: #presencefetch

[117]: #presencesubscribe

[118]: #presenceget

[119]: #presencegetall

[120]: #presencestatuses

[121]: #presenceactivities

[122]: #presenceeventpresenceselfchange

[123]: #presencegetself

[124]: #presenceeventpresencechange

[125]: #presenceupdate

[126]: https://developer.mozilla.org/en-US/docs/Web/API/fetch

[127]: https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise

[128]: https://developer.mozilla.org/docs/Web/Guide/HTML/HTML5
